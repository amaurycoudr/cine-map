...
import { DEFAULT_ERRORS, ERRORS } from 'src/utils/errors';
...
export class MapsController {
  ...
  update() {
    return tsRestHandler(contract.patchMaps, async ({ params: { id }, body }) => {
      ...
      if (code === ERRORS.NOT_FOUND) return DEFAULT_ERRORS.notFound;
      if (code === ERRORS.NOT_EDITABLE) return { body: { error: 'not editable map' }, status: 400 };
  ...
  findOne() {
    return tsRestHandler(contract.getMap, async ({ params: { id } }) => {
      ...
      if (!result) return DEFAULT_ERRORS.notFound;

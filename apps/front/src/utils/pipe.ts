...
export function pipe<A, B, C, D>(value: A, op1: (input: A) => B, op2: (input: B) => C, op3: (input: C) => D): D;

export function pipe<A, B, C, D, E>(value: A, op1: (input: A) => B, op2: (input: B) => C, op3: (input: C) => D, op4: (input: D) => E): E;
...
export function pipe(input: unknown, ...operations: ReadonlyArray<LazyOp | ((value: any) => unknown)>): any {
  let output = input;
  ...
  while (operationIndex < operations.length) {
    ...
    while (true) {
      ...
      const shouldExitEarly = _processItem(result.value, accumulator, lazySequence);
      if (shouldExitEarly) {
        break;
      }
    }
...
function _processItem(
  ...
  let isDone = false;
  for (let operationsIndex = 0; operationsIndex < lazySequence.length; operationsIndex++) {
    ...
    if (lazyResult.hasNext) {
      if (lazyResult.hasMany ?? false) {
        for (const subItem of lazyResult.next as ReadonlyArray<unknown>) {
          const subResult = _processItem(subItem, accumulator, lazySequence.slice(operationsIndex + 1));
          if (subResult) {
            return true;
          }
        }
